/* A 5x7 bitmap font for the in-game UI (item names, the search box, the
   inventory header). Five bytes per glyph, one byte per column, bit n of a
   column = row n from the top. Covers printable ASCII 32..126. */
#ifndef FONT_H
#define FONT_H
...
static const uint8_t g_font5x7[FONT_LAST - FONT_FIRST + 1][FONT_W] = {
...
/* Draws one glyph at (x,y) into an ARGB surface. `sc` scales it up. */
static void font_glyph(uint32_t *dst, int dw, int dh, int x, int y, char ch,
                       uint32_t col, int sc) {
    if ((unsigned char)ch < FONT_FIRST || (unsigned char)ch > FONT_LAST) ch = '?';
    const uint8_t *g = g_font5x7[(unsigned char)ch - FONT_FIRST];
    for (int c = 0; c < FONT_W; c++)
        for (int r = 0; r < FONT_H; r++) {
            if (!(g[c] & (1 << r))) continue;
            for (int sy = 0; sy < sc; sy++)
                for (int sx = 0; sx < sc; sx++) {
                    int px = x + c * sc + sx, py = y + r * sc + sy;
...
/* Draws a string, with a one-pixel drop shadow for legibility over the world. */
static void font_text(uint32_t *dst, int dw, int dh, int x, int y,
                      const char *s, uint32_t col, int sc) {
    int cx = x;
    for (const char *p = s; *p; p++) {
        font_glyph(dst, dw, dh, cx + sc, y + sc, *p, 0xff101010u, sc);
        font_glyph(dst, dw, dh, cx, y, *p, col, sc);
        cx += (FONT_W + 1) * sc;
    }
}
